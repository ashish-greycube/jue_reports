...
def get_car_serial_no(filters):
	if filters=={}:
		filters.update({"customs_card_type": filters.get("customs_card_type"),"supplier": filters.get("supplier"),"warehouse":filters.get("warehouse"),"serialno":filters.get("serialno"),"Status":filters.get("Status"),"Color":filters.get("Color"),"model":filters.get("model"),"Category":filters.get("Category"),"Brand":filters.get("Brand")})
	else:
		if filters.get("customs_card_type")==None:
			filters.update({"customs_card_type": filters.get("customs_card_type")})		
...
customs_card_type as `Customs Card`,					  
...
AND 1 = case when %(customs_card_type)s IS NULL THEN 1 when ( T.`Customs Card` = %(customs_card_type)s ) then 1 ELSE 0 END					  
